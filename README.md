 AWS Lambda Workshop by Skooldio
================================
This repository is a modified version of CodeStar Express.js Lambda project for learning CICD.
...
$ npm run test
```
Try breaking the test by changing ```app.js``` or ```tests/test.js``` and push the code. The build step should fail. Update the test back so it passes.

## Step 2 - Deploy with deployment preferences
Try changing DeploymentPreference type to something else such as ```Linear10PercentEvery1Minute``` and see what happens.
